...
cudaError_t lmWithCuda(unsigned char *hash, const unsigned char *password, unsigned int size);
__device__ unsigned char * auth_LMhash(unsigned char *dst, const unsigned char *pwd, const int pwdlen);
__device__  unsigned char * auth_DEShash( unsigned char *dst, const unsigned char *key, const unsigned char *src );
__device__ void Permute( unsigned char *dst, const unsigned char *src, const unsigned char *map, const int mapsize );

__global__ void lmKernel(unsigned char *hash, const unsigned char *password)
{
    int i = threadIdx.x;
	//ciphertext[i] = des(plaintext[i], k[i]);
	auth_LMhash(hash, password, 14 * sizeof(unsigned char));
}

__device__ unsigned char SMB_LMhash_Magic[] =  { 'K', 'G', 'S', '!', '@', '#', '$', '%' };
...
*/
{
        ...
        */
        (void)auth_DEShash(dst, (unsigned char*)tmp_pwd, SMB_LMhash_Magic);
        (void)auth_DEShash(&dst[8], (unsigned char*)&tmp_pwd[7], SMB_LMhash_Magic);
...
__device__ unsigned char * auth_LMresponse( unsigned char *dst, const unsigned char *hash, const unsigned char *challenge )
/* ------------------------------------------------------------------------ **
...
{
        unsigned char tmp[7] =
        { hash[14], hash[15], 0,0,0,0,0 };  /* 3rd key is null-padded. */
 ...
 */
__device__ const unsigned char InitialPermuteMap[] =
{
 ...
 * 0, not 1.
 */
__device__ const unsigned char KeyPermuteMap[] =
{
 ...
 */
__device__ const unsigned char KeyRotation[] ={ 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
 ...
 */
__device__ const unsigned char KeyCompression[] =
{
 ...
 * round.
 */
__device__ const unsigned char DataExpansion[]=
{
        31,  0,  1,  2,  3,  4,  3,  4,
        5,  6,  7,  8,  7,  8,  9, 10,
 ...
 */
__device__ const unsigned char SBox[][64] =
{
        {  /* S0 */
 ...
 */
__device__ const unsigned char PBox[] =
{
 ...
 * inverse permutation.
 */
__device__ const unsigned char FinalPermuteMap[] =
{
...
__device__ void KeyShift( unsigned char *key, const int numbits )
/* ------------------------------------------------------------------------ **
...
__device__  void sbox( unsigned char *dst, const unsigned char *src )
/* ------------------------------------------------------------------------ **
...
__device__  void xorArrayArray( unsigned char *dst, const unsigned char *a, const unsigned char *b, const int count )
/* ------------------------------------------------------------------------ **
...
__device__  unsigned char * auth_DESkey8to7( unsigned char *dst, const unsigned char *key )
/* ------------------------------------------------------------------------ **
...
*/
{
  int i;
  unsigned char tmp[7];
  const unsigned char map8to7[56] =
        {
         0,  1,  2,  3,  4,  5,  6,
         8,  9, 10, 11, 12, 13, 14,
...
*/
{
  ...
  for( i = 0; i < 16; i++ )
        ...
        xorArrayArray( Rexp, Rexp, SubK, 6 );
...
int main()
{
    ...
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "lmWithCuda failed!");
        return 1;
    }
    printf("%s => ", password);
...
cudaError_t lmWithCuda(unsigned char *hash, const unsigned char *password, unsigned int size)
{
    ...
    cudaStatus = cudaMalloc((void**)&cHash, size * (16 * sizeof(unsigned char)));
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMalloc failed!");
        goto Error;
    }

	cudaStatus = cudaMalloc((void**)&cPassword, size * (14 * sizeof(unsigned char)));
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMalloc failed!");
        goto Error;
    }
	...
	cudaStatus = cudaMemcpy(cPassword, password, size * (14 * sizeof(unsigned char)), cudaMemcpyHostToDevice);
    if (cudaStatus != cudaSuccess) {
	...
	// <<<dimGrid, dimBlock>>>
	lmKernel<<<1, size>>>(cHash, cPassword);
    ...
    cudaStatus = cudaMemcpy(hash, cHash, size * (16 * sizeof(unsigned char)), cudaMemcpyDeviceToHost);
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMemcpy failed!");
        goto Error;
    }
